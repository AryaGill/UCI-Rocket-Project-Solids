...
class ABGraph(QWidget):
    """
    Reusable widget for displaying deployment data with modern styling.
    """
    ...
    def __init__(self, parent=None):
        ...
        self.axes.set_title('Deployment', fontsize=16, fontweight='bold', 
                           color='#ff35c6', pad=15)
        self.axes.set_xlabel('Time (s)', fontsize=12, color='#b0b0b0')
        self.axes.set_ylabel('Deployment %', fontsize=12, color='#b0b0b0')
        self.axes.grid(True, alpha=0.2, linestyle='--', linewidth=0.5)
        
        # Stylish line plot with warm color for deployment
        self.line, = self.axes.plot([], [], color="#ff35c6", linewidth=2.5, 
                                    label='Deployment', antialiased=True)
        
        # Style the legend
        legend = self.axes.legend(facecolor='#2d2d2d', edgecolor='#ff35c6', 
                                 fontsize=10)
    ...
    def _update_stats(self):
        if not self.ab_data:
            ...
            return
        self._stats_text.set_text(f"Now: {self.ab_data[-1]:+.2f} %")
        
    def update_data(self, time_value, ab_value, max_points=100):
        """Update the graph with new data point."""
        self.time_data.append(time_value)
        self.ab_data.append(ab_value)

        self.time_data = self.time_data[-max_points:]
        self.ab_data = self.ab_data[-max_points:]
        
        self.line.set_xdata(self.time_data)
        self.line.set_ydata(self.ab_data)
